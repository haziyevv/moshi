...
Pipeline overview:
  1. [generate_dialogue_scripts.py]  Generate dialogue text
  2. [This script]                   Synthesize each turn → mono WAVs
  3. [build_stereo_dataset.py]       Combine into stereo WAVs + manifest
...
def synthesize_turn(model, text: str, speaker_id: str, seed: int = 1986) -> tuple[np.ndarray, int]:
...
def main():
    ...
    parser.add_argument("--seed", type=int, default=1986,
                        help="Random seed for CosyVoice reproducibility")
    ...
    for di, dialogue_dir in enumerate(dialogue_dirs):
        ...
        for turn in turns:
            ...
            try:
                audio, sr = synthesize_turn(model, text, speaker_id, seed=args.seed)
