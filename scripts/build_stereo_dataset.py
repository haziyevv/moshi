...
Each dialogue becomes one stereo WAV file (no chunking — dialogues should
already be short, ≤30s, from the generation step).

The output is directly compatible with preencode_dataset.py.

Pipeline overview:
  1. [generate_dialogue_scripts.py]  Generate dialogue text (short, ≤30s)
  2. [synthesize_dialogues.py]       Synthesize each turn with CosyVoice
  3. [This script]                   Combine into stereo WAVs + alignments + manifest
...
def generate_alignment_from_text(turn_info: list[dict]) -> list:
    ...
    turn, we distribute words evenly within each turn's time window.
    Less accurate than Whisper but requires no extra model.
    """
    alignments = []
